...
|DFS with iterative deepening | Call DFS for maximum depths 1, 2, $\ldots$ | Constant cost $c\geq 0$   | $O(b)$ | $O(b^d)$|
...
# Week 4. Markove Decision Processes
...
## Markove Decision Processes (offline)
...
* Use recurrence for policy evaluation to estimate value of the game:

\[
V_\text{eval}(s) = 
\begin{cases}
\text{Utility}(s) & \text{ IsEnd}(s)\\
\sum_{a\in\text{Actions}(s)} \pi_\text{agent}(s, a) V_\text{eval}(\text{Succ}(s, a))
\end{cases}
\]
