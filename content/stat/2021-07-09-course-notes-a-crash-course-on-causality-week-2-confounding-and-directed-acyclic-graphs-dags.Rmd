---
title: 'Course Notes: A Crash Course on Causality -- Week 2: Confounding and Directed
  Acyclic Graphs (DAGs)'
author: ''
date: '2021-07-09'
slug: course-notes-a-crash-course-on-causality-week-2-confounding-and-directed-acyclic-graphs-dags
categories:
  - Lecture notes
tags:
  - Causal Inference
...
***`r red('For the pdf slides, click [here](/pdf/070521_crash_course_Causality_week2.pdf)')`***

# Confounding

### Confounding

* `r blue('Confounders')`: variables that affect both the treatment and the outcome
...
* To control for confounders, we need to
    ...
    - `r green('Causal graphs will help answer this question')`

    2. Use statistical methods to control for these variables and estimate causal effects
...
* Encode assumption about relationship among variables
    ...
    - Tells use which variables are independent, dependent,
    conditionally independent, etc
...
* `r blue('Directed graph')`: shows that $A$ affects $Y$

```{r, echo=FALSE, engine='tikz', out.width='16%', fig.ext='png'}
...
* `r blue('Undirected graph')`: $A$ and $Y$ are associated with each other
...
* `r blue('Nodes')` or `r blue('vertices')`: $A$ and $Y$

     - We can think of them as variables
     
* `r blue('Edge')`: the link between $A$ and $Y$

* `r blue('Directed graph')`: all edges are directed

* `r blue('Adjacent')` variables: if connected by an edge
...
```{r, echo=FALSE, engine='tikz', out.width='30%', fig.ext='png'}
\begin{tikzpicture}[node distance={10mm}, thick, main/.style = {}] 
\node[main] (1) {$A$}; 
\node[main] (2) [right of=1] {$G$}; 
\node[main] (3) [right of=2] {$D$}; 
